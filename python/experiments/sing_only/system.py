...
from experiments.sing_only.model.coref import MentionMentionCNN
from experiments.sing_only.model.linking import MentionClusterEntityLinker
from experiments.sing_only.tools.evaluators import *
from experiments.sing_only.tools.ioutils import SpliceReader, StateWriter
...
class SingOnlySystem(ExperimentSystem):
    ...
    def _experiment_type(self):
        return ExperimentTypes.SING_ONLY
    ...
    def _load_transcripts(self):
        ...
        for d_in in data_in:
            ...
            for key in sorted(keys):
                if key in d_trn:
                    self.trn_coref_states.append(SingOnlyCorefState(d_trn[key], extract_gold=True))
                if key in d_dev:
                    self.dev_coref_states.append(SingOnlyCorefState(d_dev[key], extract_gold=True))
                if key in d_tst:
                    self.tst_coref_states.append(SingOnlyCorefState(d_tst[key], extract_gold=True))
    ...
    def run_coref(self, eval_only=False):
    ...
    def extract_learned_coref_features(self, save_feats=True):
        eftdims, mftdim, pftdim = self._get_coref_feature_shapes()
    ...
    def run_entity_linking(self):
        ...
        model = MentionClusterEntityLinker(self.coref_params["number_of_filters"],
                                           mrepr_dim,
                                           mpair_dim,
                                           self.linking_labels,
        ...
        results_file = "sing-only-linking-results.txt"
