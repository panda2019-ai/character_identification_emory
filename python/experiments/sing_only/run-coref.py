...
from experiments.sing_only.tools.ioutils import SpliceReader
...
def main():
    ...
    for d_in in data_in:
        ...
        for key in sorted(keys):
            if key in d_trn:
                Strn.append(SingOnlyCorefState(d_trn[key], extract_gold=True))
            if key in d_dev:
                Sdev.append(SingOnlyCorefState(d_dev[key], extract_gold=True))
            if key in d_tst:
                Stst.append(SingOnlyCorefState(d_tst[key], extract_gold=True))
    ...
    eftdims = map(lambda x: x.shape, efts)
    mftdim, pftdim = len(mft), len(Strn[0].pfts[m1][m2])

    model = MentionMentionCNN(eftdims, mftdim, pftdim, nb_fltrs, gpu=gpu)
    if not eval_only:
        # Model training
    ...
    # p, r, f = MucEvaluator().evaluate_documents(golds, autos)
    # print('MUC - %.4f/%.4f/%.4f' % (p, r, f))
